...
@RestController
@RequestMapping("/live-quiz")

public class LiveQuizController {
    @Autowired
    QuizService quizService;
    private static final Logger logger = LoggerFactory.getLogger(LiveQuizController.class);

    @GetMapping("/questions")
    public ResponseEntity<APIResponse<QuizDTO>> getQuestions(@RequestParam String email) {
        logger.info("Controller: Fetching questions for live quiz");
        try{
            QuizDTO quiz = quizService.getLiveQuiz(email);
            if(quiz == null){
                logger.error("Controller: No live quiz available currently");
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(APIResponse.error("No live quiz available", "No live quiz available"));
            }
    ...
    public ResponseEntity<APIResponse<AnswerResponseDTO>> getAnswers(@RequestParam String email, @RequestBody AnswerRequestDTO request) {
        ...
        try{
            return ResponseEntity.ok().body(APIResponse.success("Answers Fetched", quizService.getAnswers(request.getQuizId(), request.getAnswer(), email, "LIVE")));
        } catch (Exception e){
